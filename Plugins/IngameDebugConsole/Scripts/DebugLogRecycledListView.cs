...
namespace IngameDebugConsole
{
    public class DebugLogRecycledListView : MonoBehaviour
    {
        ...
        public void OnCollapsedLogEntryAtIndexUpdated(int index)
        {
            ...
            {
                var logItem = GetLogItemAtIndex(index);
                logItem.ShowCount();
        ...
        public void OnLogEntriesRemoved(int removedLogCount)
        {
            ...
            {
                var isSelectedLogEntryRemoved = isCollapseOn ? selectedLogEntry.count == 0 : indexOfSelectedLogEntry < removedLogCount;
            ...
            else
            {
                ...
                var firstVisibleLogItemInitialYPos = visibleLogItems[0].Transform.anchoredPosition.y;

                for (var i = 0; i < visibleLogItems.Count; i++)
                {
                    var logItem = visibleLogItems[i];
        ...
        {
            for (var i = Mathf.Min(startIndex, entriesToShow.Count - 1); i >= 0; i--)
            {
        ...
        private void UpdateLogItemContentsBetweenIndices(int topIndex, int bottomIndex, int logItemOffset)
        {
            ...
            {
                var logItem = visibleLogItems[i - logItemOffset];
                logItem.SetContent(entriesToShow[i], timestampsOfEntriesToShow != null ? timestampsOfEntriesToShow[i] : null, i, i == indexOfSelectedLogEntry);
