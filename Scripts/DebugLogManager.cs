...
namespace IngameDebugConsole
{
    ...
    public class DebugLogManager : MonoBehaviour
    {
        ...
        [Header("Properties")] [SerializeField] [HideInInspector] [Tooltip("Minimum height of the console window")]
        ...
        [SerializeField] [HideInInspector] [Tooltip("If enabled, the console window will continue receiving logs in the background even if its GameObject is inactive. But the console window's GameObject needs to be activated at least once because its Awake function must be triggered for this to work")]
        ...
        [SerializeField] [HideInInspector] [Tooltip("Console keeps track of the previously entered commands. This value determines the capacity of the command history (you can scroll through the history via up and down arrow keys while the command input field is focused)")]
        ...
        private static DynamicCircularBuffer<QueuedDebugLogEntry> queuedLogEntries;
        private DynamicCircularBuffer<DebugLogEntryTimestamp> queuedLogEntriesTimestamps;
        private static object logEntriesLock;
        private static int pendingLogToAutoExpand;
        ...
        private void LateUpdate()
        {
            ...
            if (uncollapsedLogEntries.Count >= maxLogCount)
            {
                ...
                var numberOfLogsToRemove = Mathf.Min(!IsLogWindowVisible ? logsToRemoveAfterMaxLogCount : uncollapsedLogEntries.Count - maxLogCount + logsToRemoveAfterMaxLogCount, uncollapsedLogEntries.Count);
        ...
        private char OnValidateCommand(string text, int charIndex, char addedChar)
        {
            if (addedChar == '\t') // Autocomplete attempt
            {
                if (!string.IsNullOrEmpty(text))
                {
                    ...
                    var autoCompletedCommand = DebugLogConsole.GetAutoCompleteCommand(commandInputFieldAutoCompleteBase, text);
        ...
        public void ReceivedLog(string logString, string stackTrace, LogType logType)
        {
            ...
            lock (logEntriesLock)
            {
                ...
                {
                    var removedLogType = queuedLogEntries.RemoveFirst().logType;
        ...
        private void ProcessLog(QueuedDebugLogEntry queuedLogEntry, DebugLogEntryTimestamp timestamp)
        {
            ...
            var isEntryInCollapsedEntryList = collapsedLogEntriesMap.TryGetValue(logEntry, out existingLogEntry);
        ...
        private void RemoveOldestLogs(int numberOfLogsToRemove)
        {
            ...
            var logEntryToSelectAndFocus = indexOfLogEntryToSelectAndFocus >= 0 && indexOfLogEntryToSelectAndFocus < logEntriesToShow.Count ? logEntriesToShow[indexOfLogEntryToSelectAndFocus] : null;
            ...
            if (removedLogEntriesToShowCount > 0)
            {
                ...
                {
                    for (var i = Mathf.Min(indexOfLogEntryToSelectAndFocus, logEntriesToShow.Count - 1); i >= 0; i--)
                    {
        ...
        private void FilterLogs()
        {
            ...
            {
                var targetLogEntries = isCollapseOn ? collapsedLogEntries : uncollapsedLogEntries;
                var targetLogEntriesTimestamps = isCollapseOn ? collapsedLogEntriesTimestamps : uncollapsedLogEntriesTimestamps;
                ...
                else
                {
                    ...
                    for (int i = 0, count = targetLogEntries.Count; i < count; i++)
                    {
                        var logEntry = targetLogEntries[i];
        ...
        public string GetAllLogs()
        {
            ...
            var count = uncollapsedLogEntries.Count;
            var length = 0;
            var newLineLength = Environment.NewLine.Length;

            for (var i = 0; i < count; i++)
            {
                var entry = uncollapsedLogEntries[i];
